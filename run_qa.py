...
parser = argparse.ArgumentParser(
    description='Run QA against the knowledge graphs')
...
def answer_complex_question(index_creator: GraphIndexCreator, question: str, prompt: str, knowlege_graph: NetworkxEntityGraph):
    ...
    print(
        f"Questions are divided into {len(kg_queries)} KG queries: {kg_queries}\n")
    question_asking_llm = OpenAI()
    sub_questions = []
    last_answer = ""
    qa_graph_chain = GraphQAChain.from_llm(
        OpenAI(), graph=knowlege_graph, verbose=True)
